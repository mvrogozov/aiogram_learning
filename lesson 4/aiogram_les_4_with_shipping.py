...
PRICES = [
    types.LabeledPrice(label='Real time machine', amount=4200000),
    types.LabeledPrice(label='Red box packing', amount=30000)
]


TELEPORTER_SHIPPING_OPTION = types.ShippingOption(
    id='teleporter',
    title='Worldwide* teleport'
).add(types.LabeledPrice('Teleport', 100000))
...
RUSSIAN_POST_SHIPPING_OPTION.add(types.LabeledPrice(
    'Wooden box with ammortizing inside', 50000
)
)

RUSSIAN_POST_SHIPPING_OPTION.add(
    types.LabeledPrice('Express shipping (5-10 days)', 70000)
)

PICKUP_SHIPPING_OPTION = types.ShippingOption(
    id='pickup', title='Self pickup'
).add(types.LabeledPrice('Self pickup in Moscow', 10000))
...
async def process_pre_checkout_query(pre_checkout_query: types.PreCheckoutQuery):
    ...
    if hasattr(pre_checkout_query.order_info, 'email') and (
        pre_checkout_query.order_info.email == 'vasya@pupkin.com'
        ):
